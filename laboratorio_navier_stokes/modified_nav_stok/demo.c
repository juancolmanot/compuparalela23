...
static void react_automatic(float* d, float* u, float* v, float last_time) {
    ...
    if (wtime() - last_time > 1) {
        i_idx = rand() % (N + 2);
        j_idx = rand() % (N + 2);
        cd = (2.f * ((float) rand()) / ((float) (RAND_MAX))) - 1.f;
        cu = (2.f * ((float) rand()) / ((float) (RAND_MAX))) - 1.f;
        cv = (2.f * ((float) rand()) / ((float) (RAND_MAX))) - 1.f;
        d[IX(i_idx, j_idx)] = source * 5.0f * cd;
        u[IX(i_idx, j_idx)] = force * 5.0f * cu;
        v[IX(i_idx, j_idx)] = force * 5.0f * cv;
...
static void idle_func(unsigned int num_clicks)
{
    static unsigned int clicks_performed = 0;   
    static int times = 1;
    ...
    start_t = wtime();
    react_automatic(dens_prev, u_prev, v_prev, last_time);
    ...
    if (clicks_performed < num_clicks) {
        clicks_performed++;
    }
    else if (clicks_performed == num_clicks) {
        glutLeaveMainLoop();
    }
...
void idle_func_with_clicks() {
    static unsigned int clicks = 1000;
    idle_func(clicks);
}
...
static void open_glut_window()
{
    ...
    glutIdleFunc(idle_func_with_clicks);
...
int main(int argc, char** argv)
{
    ...
    fp = fopen("../datafiles/test_run.dat", "w");
